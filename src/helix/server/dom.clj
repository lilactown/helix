...
(defn normalize-css-value
  [key value]
  (cond
    (contains? unitless-css-props key) (escape-html (string/trim (to-str value)))
    (number? value) (str value (when (not= 0 value) "px"))
    :else (escape-html (string/trim (to-str value)))))
...
(defprotocol IHtmlRenderable
  (-render-html [x state sb]))


(declare render-element!)


(extend-protocol IHtmlRenderable
  Element
  (-render-html [el *state sb]
    (render-element! el *state sb))
...
(defn render-element!
  "Render an element as a HTML element."
  [element *state sb]
  (let [{:keys [type props]} element
    ...
    (cond
      ...
      (do
        ...
        (when-some [type (:type props)]
          (append! sb " type=\"" (escape-html (to-str type)) "\""))

        (when (and (= "option" type)
                   (= (get-value props) *select-value*))
          (append! sb " selected=\"\""))

        (when id
          (append! sb " id=\"" id "\""))

        (render-attrs! type props sb)
        ...
        (if (= "select" type)
          (binding [*select-value* (get-value props)]
            (render-content! type props children *state sb))
          (render-content! type props children *state sb))))))
